...
#ifdef	_WX_WX_H_
	m_aThrds = (CMultiChildThread**) kmalloc(sizeof(CMultiChildThread**) * m_cThreads);
#else	// not _WX_WX_H_
	m_aThrdInfo = (MULTI_THRD_INFO*) kmalloc(sizeof(MULTI_THRD_INFO) * m_cThreads);
	m_ahsemDone = (HANDLE*) kmalloc(sizeof(HANDLE) * m_cThreads);
#endif	// _WX_WX_H_
