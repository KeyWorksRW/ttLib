...
LRESULT CMainFrame::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	ttASSERT(m_hMapping);
	if (!m_hMapping) {
		tt::MsgBox(IDS_FILE_MAPPING_ERROR);
		ExitProcess((UINT) -1);
	}
	...
	m_hWndClient = m_view.Create(m_hWnd, rcDefault, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN |
		WS_HSCROLL | WS_VSCROLL | ES_AUTOHSCROLL | ES_AUTOVSCROLL | ES_MULTILINE | ES_NOHIDESEL | ES_SAVESEL, WS_EX_CLIENTEDGE);
...
void CMainFrame::OnSaveAs()
{
	...
	if (GetSaveFileName(&ofn)) {
		...
		if (hf == INVALID_HANDLE_VALUE) {
			tt::MsgBoxFmt(IDS_CANNOT_OPEN, MB_OK | MB_ICONWARNING, ofn.lpstrFile);
			return;
		}
			...
			if (cb < (LRESULT) sizeof(szLine)) {
				*pcb = sizeof(szLine);
				...
				if (!WriteFile(hf, szLine, (DWORD) cb, &cbWritten, NULL)) {
					ttReportLastError();
					tt::MsgBoxFmt(IDS_WRITE_ERROR, MB_OK | MB_ICONWARNING, ofn.lpstrFile);
					return;
				}
				// Append a LF
				szLine[0] = 0x0a;
		...
		CloseHandle(hf);
		tt::MsgBox(IDS_FILE_SAVED, MB_OK);
	}
}
