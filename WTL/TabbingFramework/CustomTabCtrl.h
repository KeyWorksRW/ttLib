...
public:
    ...
    CCustomTabItem(const CCustomTabItem& rhs) { *this = rhs; }
...
public:
    ...
    _CSTRING_NS::CString GetText() const { return m_sText; }
    ...
    _CSTRING_NS::CString GetToolTip() const { return m_sToolTip; }
...
public:
    ...
    bool UsingText() const { return (m_sText.GetLength() > 0); }
    bool UsingToolTip() const { return (m_sToolTip.GetLength() > 0); }
...
public:
    ...
    CTabViewTabItem(const CTabViewTabItem& rhs) { *this = rhs; }
...
namespace ATL
{
    template<typename E> class CAtlArray : protected ATL::CSimpleArray<E>
    {
    protected:
...
typedef ATL::CWinTraits<WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | CTCS_TOOLTIPS, 0>
    CCustomTabCtrlWinTraits;

template<class T, class TItem = CCustomTabItem, class TBase = ATL::CWindow,
         class TWinTraits = CCustomTabCtrlWinTraits>
class ATL_NO_VTABLE CCustomTabCtrl : public ATL::CWindowImpl<T, TBase, TWinTraits>, public COffscreenDrawRect<T>
{
public:
...
protected:
    void InitializeTooltips(void)
    {
        ...
        if (!m_tooltip.IsWindow())
        {
            ...
            m_tooltip.Create(m_hWnd, NULL, NULL, WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP /* | TTS_BALLOON */,
                             WS_EX_TOOLWINDOW);
            if (m_tooltip.IsWindow())
            {
...
public:
    void AcceptItemDrag(bool bRedrawEffectedArea = true)
    {
        ...
        NMCTC2ITEMS nmh = {
            { m_hWnd, this->GetDlgCtrlID(), CTCN_ACCEPTITEMDRAG }, m_iDragItemOriginal, m_iDragItem, { -1, -1 }
        };
    ...
    void CancelItemDrag(bool bRedrawEffectedArea = true)
    {
        ...
        if (m_iDragItemOriginal >= 0 && m_iDragItem >= 0 && m_iDragItemOriginal != m_iDragItem)
        {
...
// need conditional code because types don't match in winuser.h
    ...
    void BeginItemDrag(int index, POINT ptDragOrigin)
    {
        ...
        if (index >= 0)
        {
            ...
            {
                NMCTCITEM nmh = { { m_hWnd, this->GetDlgCtrlID(), CTCN_BEGINITEMDRAG },
                                  index,
    ...
    void ContinueItemDrag(POINT ptCursor)
    {
        ...
        {
            RECT rcLeftScrollZone = { rcClient.left, rcClient.top, (m_rcTabItemArea.left + CTCD_SCROLLZONEWIDTH),
                                      rcClient.bottom };
            RECT rcRightScrollZone = { (m_rcTabItemArea.right - CTCD_SCROLLZONEWIDTH), rcClient.top,
                                       rcClient.right, rcClient.bottom };

            if (::PtInRect(&rcLeftScrollZone, ptCursor) && (ectcOverflowLeft == (m_dwState & ectcOverflowLeft)))
            {
                pT->ScrollLeft(true);
...
public:
    ...
    LRESULT OnMouseMove(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
    {
        ...
        if ((m_iDragItem >= 0) && (ectcMouseInWindow == (m_dwState & ectcMouseInWindow)) &&
            (ectcDraggingItem != (m_dwState & ectcDraggingItem)) &&
            (ectcMouseDownL_TabItem == (m_dwState & ectcMouseDown)) && (m_ptDragOrigin.x != ptCursor.x))
        {
        ...
        else if (ectcMouseInWindow == (m_dwState & ectcMouseInWindow))
        {
            ...
            if (::PtInRect(&m_rcScrollLeft, ptCursor))
            {
                ...
                else if (0 == (m_dwState & ectcMouseDown) && ectcHotTrack_ScrollLeft != (m_dwState & ectcHotTrack))
                {
                    this->ClearCurrentHotTracking(true);
    ...
    LRESULT OnLButtonUp(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& /*bHandled*/)
    {
        if (m_hWnd == ::GetCapture())
        {
            ...
            {
                // Close Button
                NMCTCITEM nmh = { { m_hWnd, this->GetDlgCtrlID(), CTCN_CLOSE },
                                  m_iCurSel,
...
public:
    ...
    void DeleteItem(TItem* pOldItem)
    {
        ...
        {
            ATLTRACE(_T("!! Exception thrown in CCustomTabCtrl::DeleteItem\r\n"));
        }
#else
        delete pOldItem;
#endif
    ...
    {
        if (!m_hWnd || !::IsWindow(m_hWnd) || (ectcEnableRedraw != (m_dwState & ectcEnableRedraw)))
        {
            return;
        }
    ...
    void UpdateLayout_Default(RECT rcTabItemArea) { UpdateLayout_ScrollToFit(rcTabItemArea); }
    ...
    void DoPrePaint(RECT rcClient, LPNMCTCCUSTOMDRAW lpNMCustomDraw)
    {
        ...
        dc.PatBlt(rcClient.left, rcClient.top, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top,
                  PATCOPY);
...
public:
    ...
    WTL::CImageList& GetImageList() const { return m_imageList; }

    WTL::CToolTipCtrl GetTooltips() const { return m_tooltip; }
    ...
    UINT GetScrollDelta(void) { return ((m_dwState & ectcScrollDeltaMask) >> ectcScrollDeltaShift); }
    ...
    ScrollRepeat GetScrollRepeat(void) { return (m_dwState & ectcScrollRepeat); }

    int InsertItem(int nItem, LPCTSTR sText = NULL, int nImage = -1, LPCTSTR sToolTip = NULL,
    ...
    BOOL MoveItem(size_t nFromIndex, size_t nToIndex, bool bUpdateSelection = true, bool bNotify = true)
    {
        ...
        {
            NMCTC2ITEMS nmh = {
                { m_hWnd, this->GetDlgCtrlID(), CTCN_MOVEITEM }, (int) nFromIndex, (int) nToIndex, { -1, -1 }
            };
            ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh);
        }
    ...
    BOOL SwapItemPositions(size_t nFromIndex, size_t nToIndex, bool bUpdateSelection = true, bool bNotify = true)
    {
        ...
        if (bNotify)
        {
            NMCTC2ITEMS nmh = { { m_hWnd, this->GetDlgCtrlID(), CTCN_SWAPITEMPOSITIONS },
                                (int) nFromIndex,
                                (int) nToIndex,
                                { -1, -1 } };
            ::SendMessage(GetParent(), WM_NOTIFY, nmh.hdr.idFrom, (LPARAM) &nmh);
