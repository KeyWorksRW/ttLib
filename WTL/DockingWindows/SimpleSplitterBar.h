...
namespace dockwins
{
    template<long TThickness = 5> class CSimpleSplitterBar : public CRect
    {
    public:
        enum
        {
            sbThickness = TThickness
        };
        ...
        bool IsPtIn(const CPoint& pt) const { return (PtInRect(pt) != FALSE); }
        ...
        HCURSOR GetCursor(const CPoint& pt) const
        {
            ...
            if (IsPtIn(pt))
            {
                ...
                hCursor = ((IsHorizontal()) ? settings.HResizeCursor() : settings.VResizeCursor());
            }
            return hCursor;
        }
    ...
    template<long TThickness = 5> class CSimpleSplitterBarEx : public CSimpleSplitterBar<TThickness>
    {
    ...
    template<class T> class CSimpleSplitterBarSlider
    {
        typedef T                                   CSplitter;
